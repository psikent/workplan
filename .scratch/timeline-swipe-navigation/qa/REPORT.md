...
- **2026-09-12 用户在真实 iPhone 上复核：翻阅与滚动手感均正常，月视图到边缘翻页正常（"表现完美"）。**
  结论依据：iOS WebKit 上 Pointer Events + `touch-action` 不可靠，改用 Touch Events 后修复（见下节）。
- 触控板双指、触控笔、鼠标拖动明确不属于本功能范围，仅通过单元/DOM 测试证明其不触发（非浏览器验收项）。
- 本机无 Android 设备条件，Android 未单独实测；其 WebKit/Blink 走同一套 Touch Events 接线，行为预期一致，
  但未做设备级确认。
