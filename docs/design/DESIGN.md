...
- **Accent**: cyan `#0891b2` in light theme and `#22d3ee` in dark theme. Favicon and PWA install icons keep `#0891b2` as the central brand color and add a baked-in cyan glass gradient, directional highlight, translucent calendar face, and specular rim. The iOS Apple Touch icon uses the approved transparent calendar artwork so the system can derive a clean Clear appearance; page controls remain flat rounded-outline icons. Green, amber, coral, and slate are reserved for status and priority.
