    ...
    <section className="settings-section bark-settings-section">
      ...
      <form className="bark-settings-form" onSubmit={submitSave}>
        ...
        <div className="bark-settings-actions">
          ...
          <button className="secondary-button" type="button" disabled={testMutation.isPending || barkQuery.isLoading} onClick={() => { saveMutation.reset(); setTestResult(null); testMutation.mutate(); }}><Send />{testMutation.isPending ? "发送中…" : "发送测试推送"}</button>
        </div>
      </form>
