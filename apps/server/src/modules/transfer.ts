...
export type ExportPayload = {
  schemaVersion: 2;
  exportedAt: string;
  data: Record<(typeof version2BusinessTables)[number], Array<Record<string, unknown>>>;
};
...
export class TransferService {
  ...
  private assertShape(payload: unknown): ImportPayload {
    ...
    if (![1, 2].includes(value.schemaVersion as number) || !value.data || typeof value.data !== "object") {
      throw invalidInput("不支持的导入文件版本");
    }
    const data = value.data as Record<string, unknown>;
    const tables = value.schemaVersion === 2 ? version2BusinessTables : version1BusinessTables;
  ...
  private replace(payload: ImportPayload): void {
    ...
    const tables = payload.schemaVersion === 2 ? version2BusinessTables : version1BusinessTables;
    for (const table of tables) {
