...
When the user explicitly starts a requirements-planning session, use interactive Q&A (`AskUserQuestion`) to remind the user to switch to a stronger model before the session begins. After the spec and tickets land, offer the user the choice: stop at the docs and implement later, or start implementing now. When the session is about to enter the implementation phase — whether immediately after that choice or in a later session — use interactive Q&A (`AskUserQuestion`) to remind the user to switch to a lower-tier model first, then start implementing.
